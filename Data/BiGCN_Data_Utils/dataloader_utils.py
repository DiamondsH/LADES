...
class RumorLoader(Dataset):
    ...
    def split(self, percent=[0.5, 1.0]):
        ...
        for i in range(len(percent)):
            ...
            rst[i].instance_weights = torch.ones([len(rst[i].data_ID)], dtype=torch.float32, device=torch.device('cuda:0'))
        return rst

    def split_data(self,percent=[0.5, 1.0]):
        data_size = len(self.data_ID)
        start_end = [int(item * data_size) for item in percent]
        new_idxs = list(random.sample(list(range(data_size)), data_size))
        rst = self.__class__()
        rst.data_ID = [self.data_ID[idx] for idx in new_idxs[start_end[0]:start_end[1]]]
        rst.data_len = [self.data_len[idx] for idx in new_idxs[start_end[0]:start_end[1]]]
        rst.data_y = [self.data_y[idx] for idx in new_idxs[start_end[0]:start_end[1]]]
        rst.data = {ID: self.data[ID] for ID in rst.data_ID}
        return rst
