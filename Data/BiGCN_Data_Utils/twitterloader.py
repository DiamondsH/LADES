...
class BiGCNTwitterSet(TwitterSet):
    ...
    def collate_raw_batch(self, batch):
        seqs = [item.text for item in batch]
        TD_graphs = [item.g_TD for item in batch]
        BU_graphs = [item.g_BU for item in batch]
        labels = [item.data_y for item in batch]
        topic_labels = [item.topic_label for item in batch]
        return seqs, TD_graphs, BU_graphs, torch.tensor(labels).argmax(dim=1), torch.tensor(topic_labels)
