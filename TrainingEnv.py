...
class AugmentationModel(VirtualModel):
    ...
    def AugBatch2Vecs(self,model, batch):
        ...
        inputs = model.sent2vec.AugForward(tfidf)
        seq_outs = model.prop_model(num_nodes, A_TD, A_BU, inputs)
        return seq_outs

    def augLossAndAccV2(self,model, classifier, batch, aug_type, labels: torch.Tensor = None,
                        temperature=1.0, label_weight=None, reduction='mean'):
        # vecs = self.AugBatch2Vecs(batch, aug_type=aug_type)
        vecs = self.AugBatch2Vecs(model,batch)
...
class VirtualEvaluater:
    ...
    def __call__(self, model: VirtualModel, p_r_f1=True):
        # dataloader = self.dataset2dataloader(self.dataset,True,self.batch_size)
        ...
        # print("logits_tensor", logits_tensor)
        # print("self.labelTensor", self.labelTensor)
        ...
        # print("self.labelTensor", self.labelTensor)
        # print("self.labelTensor.dim()", self.labelTensor.dim())
        if p_r_f1:
    ...
    def training(self, trModel: VirtualModel, train_set: CustomDataset, batch_size, max_epoch, lr4model=5e-5,
        ...
        for epoch in range(max_epoch):
            for batch in self.trainset2trainloader(train_set, batch_size):
                ...
                print(
                    '####Pre Train Classifier %3d |  (%3d , %3d) ####, loss = %6.8f, Acc = %6.8f, mean_loss = %6.8f' % (
